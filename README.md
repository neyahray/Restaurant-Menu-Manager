...
## Description
This is a simple JavaFX based application, for managing a restaurant menu. The application provides an interactive interface for viewing and managing menu items categorized into Pastries and Drink. It helps staff organize food and drinks without using paper.
...
* **Validation Algorithms:**: Before saving anything to the database, the system checks that the input is correct. For example, that the name doesn't contain weird symbols, and that the price is actually a number.
...
* **HashMaps:** Used in the `LoginPageController` to quickly match a username to a password. Instead of searching through everyone, it finds the right person instantly. 
* **Inheritance:** The project uses a hierarchical structure where `Pastry` and `Drink` inherit from an abstract `Item` class, utilizing a common data model across the app.
...
* **`CommonMethods`**: A utility module containing shared UI logic like `playScaleAnimation` and `notification` systems, preventing code duplication.
* **`CommonStyledItem`**: A helper class that takes raw data and turns it into a styled, clickable UI element.
...
## Presentation
Here: https://drive.google.com/drive/folders/1ZgPWXbj1dx_UKPkO8BQ0H9qppffGNZzb?usp=sharing